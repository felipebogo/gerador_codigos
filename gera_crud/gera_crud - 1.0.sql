    ...
    BEGIN
      ...
      FOR r IN cColunas LOOP
        ...
        IF r.data_type = 'DATE' THEN
          vSelect := vSelect || 'TO_CHAR('||lower(r.column_name)||',''DD/MM/RRRR'') ';
        ELSE
...
'  FUNCTION lst'||fGetParametrosLst(14)||' RETURN CLOB IS
  ...
  BEGIN
    BEGIN
      nPagNum := inPagNum * inPagTam + 1;
      
'||fGetQuery||'
      
      vSql := vSql||'' ORDER BY ''||NVL(ivOrder,'' 1 DESC '');
   
      vSql := ''SELECT * 
                 FROM (SELECT inner.*
                            , ROW_NUMBER() OVER(ORDER BY '' || NVL(ivOrder,''1 desc'') || '') pljson_sql_rn 
                         FROM ('' || vSql || '') inner) 
                        WHERE ((pljson_sql_rn BETWEEN :offset
                                                  AND :offset + :fetchFirst - 1
                          AND :offset IS NOT NULL
                          AND :fetchFirst IS NOT NULL)
                           OR (:offset IS NULL OR :fetchFirst IS NULL)
                              )
               '';
               
      OPEN cCur FOR vSql USING '||fGetBinds||'
                             , nPagNum
                             , nPagNum
                             , inPagTam
                             , nPagNum
...
BEGIN
  ...
  BEGIN
    ...
    /*cResult := fCrudGenerator( ivTabela   => upper('tb_aipex_prop_averbamento')
